...
# encoderR = MLP_emb(n_in=49*args.dims, n_hid=hidden_dim,n_out=hidden_dim,do_prob=0.2).to(device)
# calculator= MLPEncoderpre(49*args.dims , hidden_dim,
#                          2,
...
def train_SIjoint(epoch, encoder,decoder,emb_SI_prior,encoder_SI_prior,scheduler,train_loader,optimizer,rel_rec,rel_send,valid_loader,encoder_file,decoder_file,log,best_val_loss,args):
    t = time.time()
    nll_train = []
    acc_train = []
    kl_train = []
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        with torch.no_grad():
            h = emb_SI_prior(data)
            scores = encoder_SI_prior(h,rel_rec, rel_send)
            # scores = encoder_SI_prior(h)
            edge_probs = F.softmax(scores,dim=-1)
        logits = encoder(data, rel_rec, rel_send,h)
        ...
        loss_kl = kl_categorical(prob, edge_probs, args.num_atoms)
        # if args.prior:
    ...
    if args.save_folder and np.mean(nll_val) < best_val_loss:
        # print(np.mean(nll_val) ,best_val_loss)
...
def test_SIjoint(encoder,decoder,emb_SI_prior,encoder_SI_prior,rel_rec,rel_send,test_loader,encoder_file,decoder_file,save_folder,log,args):
    acc_test = []
    nll_test = []
    kl_test = []
    mse_test = []
    ...
    for batch_idx, (data, relations) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                h = emb_SI_prior(data_encoder)
                scores = encoder_SI_prior(h,rel_rec, rel_send)
                # scores = encoder_SI_prior(h)
                edge_probs = F.softmax(scores,dim=-1)
            logits = encoder(data_encoder, rel_rec, rel_send,h)
            edges = gumbel_softmax(logits, tau=args.temp, hard=True)
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        val_loss = train_SIjoint(epoch, best_val_loss)
