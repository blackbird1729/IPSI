...
class NRIModel_joint(nn.Module):
    """Auto-encoder."""
    def __init__(self, encoder: nn.Module, decoder: nn.Module, es: Tensor, size: int):
        ...
        """
        super(NRIModel_joint, self).__init__()
        self.enc = encoder
        self.dec = decoder
    ...
    def forward(self, states_enc: Tensor, states_dec: Tensor, pre_inputs,hard: bool=False, p: bool=False, M: int=1, tosym: bool=False):
        """
        Args:
        ...
        logits = self.enc(states_enc, pre_inputs,self.es)
        if tosym:
