...
class XNRIIns_joint(Instructor):
    """
    Training and testing for the neural relational inference task.
    """

    def __init__(self, model: torch.nn.DataParallel, model_prior,data: dict, es: np.ndarray, cmd):
        """
        Args:
        ...
        """
        super(XNRIIns_joint, self).__init__(cmd)
        self.model = model
    ...
    def train(self, save_folder, start_time):
        ...
        for epoch in range(1, 1 + self.cmd.epochs):
            ...
            for adj, states in data:
                ...
                N += scale
                loss_a += scale * self.train_nri(adj,states, self.time_steps)
            loss_a /= N
    ...
    def train_nri(self, adj,states: Tensor, time_steps) -> Tensor:
        """
        Args:
        ...
        with torch.no_grad():
            states_prior = states.permute(2, 0, 1, -1).contiguous()
            h, _, _ = getattr(self.model_prior.enc, 'reduce_' + self.model_prior.enc.reducer)(x=states_prior, es=self.es)
            prob_prior = self.model_prior.predict_relations(states)
            # print(edge_accuracy(prob_prior, adj))
        if time_steps < 49:
            output, prob = self.model(states, states, h,p=True, M=1, tosym=cfg.sym)
        else:
            output, prob = self.model(states, states, h,p=True, M=cfg.M, tosym=cfg.sym)
        prob = prob.transpose(0, 1).contiguous()
        # reconstruction loss and the KL-divergence
        loss_nll = nll_gaussian(output, states[:, 1:], 5e-5)
        loss_kl = cross_entropy(prob, prob_prior) / (prob.shape[1] * self.size)
    ...
    def test(self, name: str, M: int, save_folder: str):
        ...
        with torch.no_grad():
            for adj, states in data:
                ...
                states_prior = states_enc.permute(2, 0, 1, -1).contiguous()
                h, _, _ = getattr(self.model_prior.enc, 'reduce_' + self.model_prior.enc.reducer)(x=states_prior,
                                                                                                  es=self.es)
                prob_prior = self.model_prior.predict_relations(states_enc)

                if self.time_steps < 49:
                    output, prob = self.model(states_enc, states_dec, h,hard=True, p=True, M=1, tosym=cfg.sym)
                else:
                    output, prob = self.model(states_enc, states_dec, h,hard=True, p=True, M=M, tosym=cfg.sym)
                prob = prob.transpose(0, 1).contiguous()
                ...
                # print("relations_np.shape: ", relations_np.shape)
                ...
                target = states_dec[:, 1:]
                # print("prob_np.shape: ", prob_np.shape)
        ...
        np.save(save_folder + 'results/edges_test.npy', np.vstack(probs_test))
        print("edges_test saved at: " + save_folder + 'results/edges_test.npy')
        print("auroc: {:.4f}".format(auroc_res))
        self.log.info(
            '{} M {:02d} mse {:.3e} acc {:.4f} _acc {:.4f} rate {:.4f} ratio {:.4f} sparse {:.4f} auroc {:.4f}'.format(
