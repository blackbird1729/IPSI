...
from utils import *
from modules_SI_joint import MLPEncoder_SIjoint  # 根据你训练时选择的 encoder 类型导入
from modules_SI_prior import *

def make_pseudo_labels_iteratively(rel_rec,rel_send,train_loader,valid_loader,test_loader,args):
    # 1. 加载数据
    ...
    # # 3. 初始化 encoder 并加载权重
    #
    # encoder = MLPEncoder_SIjoint(args.timesteps * args.dims,
    #                          args.encoder_hidden,
    #                          args.edge_types,
    ...
    #
    #
    # ckpt = torch.load(os.path.join(args.load_folder, 'encoder.pt'),
    ...
    #     encoder.cuda()
    # dynamic_dim = 1
    # use_transformer = False  # 选择 RNN 或 Transformer 作为编码器
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    #
    # hidden_dim = 256
    # # **初始化模型**
    #
    # # encoderR = RNNEncoder(input_dim=dynamic_dim, hidden_dim=hidden_dim, rnn_type="GRU", bidirectional=False).to(device)
    # encoderR = MLP_emb(n_in=49 * 1, n_hid=hidden_dim, n_out=hidden_dim, do_prob=0.2).to(device)
    # calculator = MLPEncoderpre(49, hidden_dim,
    #                            2,
    #                            do_prob=0.2, factor=True).to(device)
    # # calculator=MLPScore(hidden_dim).to(device)
    #
    # # **加载保存的模型**
    # checkpoint = torch.load("saved_model/best_model.pth", map_location=device)
    #
    # encoderR.load_state_dict(checkpoint['encoder_state_dict'])
    # calculator.load_state_dict(checkpoint['score_fn_state_dict'])
    # 4. 遍历每个 split，生成伪标签
    rows, cols = np.where(off_diag)
    rows, cols = np.indices((num_atoms, num_atoms))
    rows = rows.flatten()
    cols = cols.flatten()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    emb_SI_prior = MLP_emb(n_in=args.timesteps * args.dims, n_hid=args.SI_prior_hidden_dim,
                           n_out=args.SI_prior_hidden_dim, do_prob=0.0).to(device)
    checkpoint = torch.load("SIprior_model/SIprior_best_model.pth")
    ...
    encoder_SI_joint = MLPEncoder_SIjoint(args.timesteps * args.dims, args.encoder_hidden,
                                          args.edge_types,
                                          args.encoder_dropout, args.factor).to(device)
    encoder_file = os.path.join("SIjoint_data/", 'encoder.pt')

    encoder_SI_joint.load_state_dict(torch.load(encoder_file))
    def infer_split(loader, split_name):
        emb_SI_prior.eval()
        encoder_SI_joint.eval()
        ...
        with torch.no_grad():
            for feat, relations in loader:
                ...
                if split_name == "test":
                    ...
                    h = emb_SI_prior(feat_encoder)
                    logits = encoder_SI_joint(feat_encoder, rel_rec, rel_send,h)
                else:
                    h = emb_SI_prior(feat)
                    logits = encoder_SI_joint(feat, rel_rec, rel_send,h)
                # print(logits.shape)
        ...
        # print(avg_logits)
        # print((avg_logits.argmax(dim=-1), relations[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load-folder', type=str, default= 'pseudo_model',
                        help='训练好的模型目录，包含 encoder.pt')
    parser.add_argument('--save-folder', type=str, default='pseudo_data',
    ...
    make_pseudo_labels_iteratively(args)
